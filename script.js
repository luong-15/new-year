...
// Tự động phát âm thanh (muted) khi trang load
document.addEventListener('DOMContentLoaded', () => {
    audio.muted = true; // Bắt đầu ở chế độ tắt tiếng
    audio.play().then(() => {
        // Âm thanh đã phát thành công (muted), chờ tương tác người dùng để bật tiếng
        window.addEventListener('click', function handler() {
            audio.muted = false; // Bật tiếng khi có tương tác
            window.removeEventListener('click', handler); // Gỡ bỏ listener để tránh bật tiếng nhiều lần
        });
    }).catch(error => {
        // Autoplay bị chặn hoàn toàn, hiển thị nút play cho người dùng
        console.error("Autoplay was prevented:", error);
        // playBtn.style.display = "block" // Nếu bạn muốn hiển thị nút play ngay lập tức
    });
});